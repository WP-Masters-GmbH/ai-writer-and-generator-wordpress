	...
	<?php if(!empty($queued_articles)) {
		foreach($queued_articles as $article) {
            ...
            <tr>
                ...
                <td><?php if($article->post_id > 0) { ?> <a class="wpm-button-black" href="https://ai.wp-masters.com?view_article=<?php echo esc_attr($article->article_name); ?>&activation_key=<?php if(isset($settings['activation_key'])) { echo esc_attr($settings['activation_key']); } else { echo esc_attr('FREE'); } ?>" target="_blank">View Article</a>
                        <a class="wpm-button-black" href="<?php echo esc_attr(get_home_url()."?import_ai_post={$article->id}"); ?>" target="_blank">Import Post</a> <?php } elseif($article->errors != '') { ?><a class="wpm-button-black wpm-red-btn modal-view-content" href="#modal-content" rel="modal:open" data-title="<?php echo esc_attr($article->article_name); ?>" data-article-content="<?php echo esc_html(implode(', ', $article->errors)); ?>">View Errors</a><?php } else { echo esc_html("In Process"); } ?></td>
